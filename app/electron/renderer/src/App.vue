<template>
  <div class="min-h-screen bg-slate-950 text-slate-100">
    <header class="border-b border-slate-800 px-8 py-6">
      <h1 class="text-2xl font-semibold tracking-tight">Excel Validator</h1>
      <p class="text-slate-400">Drop a spreadsheet, validate rows, export a report.</p>
    </header>
      ...
      <section class="space-y-6">
        ...
        >
          ...
          <label class="mt-4 inline-flex cursor-pointer items-center justify-center rounded-md bg-emerald-500 px-4 py-2 text-xs font-semibold text-slate-900 hover:bg-emerald-400">
            Choose File
            <input type="file" class="hidden" accept=".xlsx" @change="onFilePick" />
          </label>
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
          ...
          <p class="mt-1 text-xs text-slate-400">Select categories to create separate sheets in the report.</p>
          <div class="mt-4 space-y-2 text-sm">
            <label v-for="category in categoryOptions" :key="category" class="flex items-center gap-3">
...
const categoryOptions = ["Regular Employee", "Fee For Service"];
...
const validateFile = async () => {
  ...
  payload.append("categories", JSON.stringify(selectedCategories.value));
